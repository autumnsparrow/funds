...
public class PEMStringLoader {
	...
	private PEMReader openPEMResource(String fileName, PasswordFinder pGet)
		...
		if(fileName.startsWith("http://")){
			...
			InputStream res=new ByteArrayInputStream(fileName.getBytes(Charset.forName("utf-8")));
			//new FileInputStream(fileName);// this.getClass().getResourceAsStream(fileName);
			Reader fRd = new BufferedReader(new InputStreamReader(res));
			try {
				reader= new PEMReader(fRd, pGet);
			} catch (Exception e) {
