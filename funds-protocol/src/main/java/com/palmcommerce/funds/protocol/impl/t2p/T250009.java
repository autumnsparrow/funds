 ...
 * 6.2.9  批量汇付对账接口
 * 6.2.9.1 业务描述：
由福彩中心发起同步交易或异步交易，在日终后1小时进行批量汇付对账。
福彩中心发送对帐请求，银行方触发生成对帐文件的函数，生成对帐文件之后，
银行通过sftp方式将对帐文件上传到福彩中心的接口前置机，
银行校验上传的对帐文件是否完整，校验完整后银行方返回“成功”或“失败”应答信息；
对帐文件的格式与缴费的格式完全一样
 * @author lottery
 *
 */
@ProtocolClassMetaType(mode=TRANSFER_MODE.DELAY_PROXY)
public class T250009 extends BasicProtocol {
	

	
	public T250009(String transcode, String from, String to) {
...
校验文件大小	字符	最长10位	Y	字节数
	...
	public T250009(String transcode, String from, String to, String packet,
			boolean isRequest) {
		super(transcode, from, to, packet, isRequest);
	}


	public T250009(String transcode, String from, String to, String request,
			String response) {
	...
	@Override
	public String toString() {
		return "T250009 [request=" + request + ", response=" + response + "]";
	}
